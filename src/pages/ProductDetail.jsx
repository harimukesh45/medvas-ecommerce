...
import { Minus, Plus, MessageCircle, ArrowLeft, CheckCircle2 } from 'lucide-react';
...
function ProductDetail() {
  ...
  const handleOrder = () => {
    ...
    const phoneNumber = "919360617191"; // REPLACE WITH YOUR NUMBER
    const message = `Hello MEDVAS ENTERPRISES, I'd like to place an order.%0A%0A*Product:* ${product.name}%0A*Quantity:* ${quantity}%0A%0A*Customer Details:*%0AName: ${customerInfo.name}%0AAddress: ${customerInfo.address}`;
  ...
  return (
    <div className="container" style={{ maxWidth: '1100px' }}>
      <Link
        to="/products"
        style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', color: 'var(--text-muted)', textDecoration: 'none', marginBottom: '30px', fontWeight: '600' }}
      >
      ...
      <div className="card product-detail-layout" style={{ display: 'flex', gap: '50px', padding: '40px' }}>
        ...
        <div className="product-image-container" style={{ flex: '1', minWidth: '400px' }}>
          <div style={{ width: '100%', aspectRatio: '1/1', backgroundColor: '#E2E8F0', borderRadius: '12px', overflow: 'hidden', border: '1px solid var(--border-color)' }}>
            <img
          ...
          <div style={{ padding: '25px', backgroundColor: 'var(--bg-main)', borderRadius: '12px', marginTop: 'auto', border: '1px solid var(--border-color)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '25px', flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', border: '2px solid var(--border-color)', borderRadius: '8px', backgroundColor: '#fff' }}>
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))} style={{ padding: '10px 15px', border: 'none', background: 'none', cursor: 'pointer' }}>
                  <Minus size={18} />
                </button>
                <span style={{ padding: '0 15px', fontWeight: '700', fontSize: '1.1rem' }}>{quantity}</span>
                <button onClick={() => setQuantity(quantity + 1)} style={{ padding: '10px 15px', border: 'none', background: 'none', cursor: 'pointer' }}>
                  <Plus size={18} />
                </button>
              </div>
            </div>
